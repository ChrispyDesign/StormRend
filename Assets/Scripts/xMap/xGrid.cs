...
namespace StormRend.Defunct
{
	public class xGrid
	{
		public static xTile[,] m_nodes;

		[SerializeField] private xSpawnManager m_spawnManager;
		...
		public xGrid(Transform _prefab, int _nodeSize, Transform _parent, NodeType[,] _gridData)
		{
			m_parent = _parent;
		...
		void CreateGrid(Transform _prefab)
		{
			...
			for (int x = 0; x < m_gridSize.x; x++)
			{
				for (int y = 0; y < m_gridSize.y; y++)
				{
					...
					tile.name = "(" + x + ", " + y + ")";
					m_nodes[x, y] = tile.GetComponent<xTile>().SetNodeVariables(pos, new Vector2Int(x, y), m_gridData[x, y]);
					...
					foreach (xPlayerUnit player in gm.GetPlayerUnits())
					{
					...
					foreach (xEnemyUnit enemy in gm.GetEnemyUnits())
					{
						if (enemy.coords.x == x &&
						   enemy.coords.y == y)
		...
		xTile[] GenerateNeighbours(int _x, int _y)
		{
			xTile[] neighbours = new xTile[4];
		...
		public static xTile GetNodeFromCoords(int _x, int _y) { return m_nodes[_x, _y]; }
		public static xTile CoordToTile(Vector2Int _coords) { return m_nodes[_coords.x, _coords.y]; }
	}
}
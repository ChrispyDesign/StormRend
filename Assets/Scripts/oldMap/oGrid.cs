...
namespace StormRend.Defunct
{
	public class oGrid
	{
		public static oTile[,] m_nodes;

		[SerializeField] private oSpawnManager m_spawnManager;
		...
		public oGrid(Transform _prefab, int _nodeSize, Transform _parent, NodeType[,] _gridData)
		{
			m_parent = _parent;
		...
		{
			m_spawnManager = GameObject.FindObjectOfType<oSpawnManager>();

			var gm = GameManager.singleton;

			m_nodes = new oTile[m_gridSize.x, m_gridSize.y];
			//int i = 0;
			for (int x = 0; x < m_gridSize.x; x++)
			{
				for (int y = 0; y < m_gridSize.y; y++)
				{
					...
					tile.name = "(" + x + ", " + y + ")";
					m_nodes[x, y] = tile.GetComponent<oTile>().SetNodeVariables(pos, new Vector2Int(x, y), m_gridData[x, y]);
		...
		oTile[] GenerateNeighbours(int _x, int _y)
		{
			oTile[] neighbours = new oTile[4];
		...
		public static oTile GetNodeFromCoords(int _x, int _y) { return m_nodes[_x, _y]; }
		public static oTile CoordToTile(Vector2Int _coords) { return m_nodes[_coords.x, _coords.y]; }
	}
}